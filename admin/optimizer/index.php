...
echo '<script>(function(){var form=document.getElementById("optimizer-form");if(!form){return;}form.addEventListener("submit",function(event){event.preventDefault();var button=form.querySelector("button[type=submit]");if(button){button.disabled=true;}fetch(form.action,{method:"POST",body:new FormData(form),credentials:"same-origin",headers:{"X-Requested-With":"Optimizer"}}).then(function(response){if(!response.ok){throw new Error("HTTP "+response.status);}return response.text();}).then(function(html){var parser=new DOMParser();var documentFragment=parser.parseFromString(html,"text/html");var next=documentFragment.querySelector("#optimizer-content");var current=document.getElementById("optimizer-content");if(!next||!current){throw new Error("Invalid HTML response");}current.replaceWith(next);}).catch(function(error){window.alert("Optimizer save error: "+error.message);}).finally(function(){if(button){button.disabled=false;}});});})();</script>';
echo '</div></div>';